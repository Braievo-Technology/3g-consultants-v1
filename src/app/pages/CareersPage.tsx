...
const jobCategories = ['All', 'Construction', 'Engineering', 'Project Management', 'Design', 'Administration'];
const employmentTypes = ['All', 'FULLTIME', 'PARTTIME', 'CONTRACT'];
const benefits = [{
...
const CareersPage = () => {
  ...
  useEffect(() => {
    const filtered = jobs.filter(job => {
      ...
      const matchesEmploymentType = activeEmploymentType === 'All' || job.employment_type === activeEmploymentType;
  ...
  }, [activeCategory, activeEmploymentType, searchQuery, jobs]);
  if (loading) {
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="mb-8">
            <div className="relative max-w-md mx-auto mb-8">
              <input type="text" placeholder="Search positions..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full px-4 py-3 pl-12 rounded-lg border border-[#f1c235]" />
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {jobCategories.map(category => <button key={category} onClick={() => setActiveCategory(category)} className={`px-4 py-2 rounded-full transition-colors duration-200 ${activeCategory === category ? 'bg-[#f1c235] text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                    {category}
                  </button>)}
              </div>
            </div>
            <div className="mb-8">
              ...
              <div className="flex flex-wrap gap-2">
                {employmentTypes.map(type => <button key={type} onClick={() => setActiveEmploymentType(type)} className={`px-4 py-2 rounded-full transition-colors duration-200 ${activeEmploymentType === type ? 'bg-yellow-500 text-black' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                    {type}
                  </button>)}
              </div>
            </div>
