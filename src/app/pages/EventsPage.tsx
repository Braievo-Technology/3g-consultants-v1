...
//   const filteredEvents = activeCategory === 'All' ? events : events.filter(event => event.event_type === activeCategory.toLowerCase());
//   if (loading) {
...
//           <SectionTitle title="Upcoming Events" subtitle="Join us at these upcoming events to learn, connect, and grow." centered={true} />
//           <div className="flex flex-wrap justify-center mb-12 gap-2">
//             {['All', 'Conference', 'Workshop'].map(category => <button key={category} onClick={() => setActiveCategory(category)} className={`px-6 py-2 rounded-full transition-colors duration-200 ${activeCategory === category ? 'bg-[#f1c235] text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
//                 {category}
//               </button>)}
//           </div>
//           <div className="space-y-6 mt-12">
//             {filteredEvents.map(event => <div key={event.id} className="bg-gray-50 rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300">
//                 <div className="flex flex-col md:flex-row h-full">
//                   {event.images && event.images[0] && <div className="md:w-1/3 h-[200px] md:h-auto relative">
//                       <img src={event.images[0].image_name} alt={event.title || ''} className="w-full h-full object-cover" />
//                     </div>}
