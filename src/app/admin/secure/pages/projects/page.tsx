...
/*    const [filteredProjects, setFilteredProjects] = useState<ApiProject[]>([])
    ...
    const [ setFilteredProjects] = useState<ApiProject[]>([])
...
/*    const handleAddProject = async (data: ProjectFormData) => {
        try {
            const formattedData = {
                ...
                status: data.status,
                budget: data.budget ? parseFloat(String(data.budget)) : 0,
...
/*    const handleUpdate = async (data: ProjectFormData) => {
        ...
        try {
            const formattedData = {
                ...
                status: data.status,
                budget: parseFloat(String(data.budget)),
                category: data.category,
                images: data.images,
            }
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    ...
    return (
        <PageTransition>
            <div className="px-4 py-6 md:px-6 lg:px-8">
                ...
                {/*        setEditingProject(null)*/}
                {/*    }}*/}
                {/*    title={editingProject ? 'Edit Project' : 'Add New Project'} children={undefined}                >*/}
                {/*  /!*  <ProjectForm*/}
