...
import { BuildingIcon, DropletIcon, LeafIcon, HomeIcon, BuildingIcon as UrbanIcon, Briefcase as BriefcaseIcon, MapIcon, ChevronLeftIcon, ChevronRightIcon } from 'lucide-react';
const services = [{
    ...
    perspective: '1000px'
      ...
      <Link href="/services" className="block h-full">
        ...
        <div className="relative h-full p-6 md:p-8 flex flex-col justify-between z-10">
          ...
          <motion.div className="flex items-center text-yellow-400 font-medium text-xl" initial={{
          x: 0
        }} whileHover={{
          x: 5
        }} transition={{
...
const MobileServiceCard = ({
  ...
  return <motion.div className={`absolute inset-0 w-full h-full ${isActive ? 'z-20' : 'z-10'}`} initial={{
    opacity: 0,
    x: 100
  }} animate={{
    ...
    duration: 0.3
      ...
      <div className="relative h-full">
        ...
        <div className="relative h-full p-6 flex flex-col justify-end z-10">
          ...
          <Link href="/services" className="inline-flex items-center text-yellow-400 font-medium">
            Learn more
...
const ServicesSection = () => {
  ...
  return <section className="relative h-screen w-full overflow-hidden flex items-center" style={{
    backgroundColor: '#000000'
      ...
      background: ['radial-gradient(circle at 0% 0%, rgba(241, 194, 53, 0.08) 0%, transparent 50%)', 'radial-gradient(circle at 100% 100%, rgba(241, 194, 53, 0.08) 0%, transparent 50%)', 'radial-gradient(circle at 0% 100%, rgba(241, 194, 53, 0.08) 0%, transparent 50%)', 'radial-gradient(circle at 100% 0%, rgba(241, 194, 53, 0.08) 0%, transparent 50%)', 'radial-gradient(circle at 0% 0%, rgba(241, 194, 53, 0.08) 0%, transparent 50%)']
    }} transition={{
      duration: 20,
      repeat: Infinity,
      ease: 'linear'
    }} />
      {isMobile ? <div className="relative w-full h-full" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
          <AnimatePresence mode="wait">
            <MobileServiceCard key={currentSlide} service={services[currentSlide]} isActive={true} />
          </AnimatePresence>
          <div className="absolute bottom-8 left-0 right-0 z-30 flex justify-center items-center gap-4">
            <button onClick={prevSlide} className="p-2 rounded-full bg-white/10 backdrop-blur-sm text-white hover:bg-white/20 transition-colors">
              <ChevronLeftIcon size={24} />
            </button>
            <div className="flex gap-2">
              {services.map((_, index) => <button key={index} onClick={() => setCurrentSlide(index)} className={`w-2 h-2 rounded-full transition-colors ${index === currentSlide ? 'bg-yellow-500' : 'bg-white/50'}`} />)}
            </div>
            <button onClick={nextSlide} className="p-2 rounded-full bg-white/10 backdrop-blur-sm text-white hover:bg-white/20 transition-colors">
              <ChevronRightIcon size={24} />
            </button>
          </div>
        </div> : <div className="relative z-10 flex flex-nowrap h-[80vh] w-full">
