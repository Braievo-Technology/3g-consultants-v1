import api from './api';
import { CreateJobApplicationDTO, JobApplication } from '../../types/jobApplication';
export const jobApplicationService = {
  createJobApplication: async (application: CreateJobApplicationDTO): Promise<JobApplication> => {
    try {
      const formData = new FormData();
      // Add basic fields
      formData.append('name', application.name);
      formData.append('email', application.email);
      formData.append('jobOpportunityId', application.jobOpportunityId.toString());
      // Add optional fields
      ...
      if (application.expected_salary) {
        formData.append('expected_salary', application.expected_salary.toString());
      }
